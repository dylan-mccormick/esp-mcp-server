...
static const std::unordered_set<uint8_t> PROHIBITED_PINS = {0, 1, 3, 6, 7, 8, 9, 10, 11, 20, 24, 28, 29, 30, 31, 34, 35, 36, 37, 38, 39};
...
void handlePinWrite(JsonObjectConst args, JsonVariant result) {
    ...
    if (hasDigital == hasAnalog) {
        writeToolError(result, "either analogValue must be provided as an integer or digitalValue must be provided as a string, but not both");
        return;
    }
    ...
    // digital processing
    if (hasDigital && !(strcasecmp("HIGH", digitalValue.c_str()) == 0 || strcasecmp("LOW", digitalValue.c_str()) == 0)) {
...
MCP_TOOL_DEF(
    "pinWrite",
    "Writes the specified value to the requested pin. GPIO pins range between 0-39; GPIO 0, 1, 3, 6-11, 20, 24, 28-31, 34-39 are prohibited. Must specify either an analogValue or a digitalValue, depending on whether to use PWM.",
    pinWriteSchema,
    handlePinWrite
);